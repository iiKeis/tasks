...
export function findQuestion(
    ...
    const findQuestionById = questions.find(
...
export function removeQuestion(questions: Question[], id: number): Question[] {
    const filterOutQuestionId = questions.filter(
        (question: Question): boolean => question.id !== id,
    );
...
export function sumPoints(questions: Question[]): number {
    ...
    const addMappedPoints = mapAllPoints.reduce(
...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    const getPublishedQuestionsPoints = mapOnlyPublishedQuestion.map(
        (q) => q.points,
    );
    const sumOnlyPublishedQuestions = getPublishedQuestionsPoints.reduce(
...
export function toCSV(questions: Question[]): string {
    ...
    const formatRow = ["id,name,options,points,published", ...getAllData].join(
        "\n",
    );
...
export function makeAnswers(questions: Question[]): Answer[] {
    const mapQuestionToAnswer = questions.map(
...
export function publishAll(questions: Question[]): Question[] {
    const nowPublishedQuestions = questions.map(
...
export function sameType(questions: Question[]): boolean {
    ...
    const everyTypeQuestion = questions.every(
        (question: Question): boolean => question.type === firstQuestion,
    );
...
export function renameQuestionById(
    ...
    const mapAllQuestions = questions.map(
