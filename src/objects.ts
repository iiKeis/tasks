...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        return question.options.some((aQuestion) => aQuestion === answer);
    }
}
...
export function toShortForm(question: Question): string {
    const combinedString = question.id + ": " + question.name.slice(0, 10);
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "multiple_choice_question") {
        const questionLines = question.options
            .map((option) => "- " + option)
            .join("\n");
...
export function renameQuestion(question: Question, newName: string): Question {
    const questionNewName = { ...question, name: newName };
    return questionNewName;
}
